  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setFilteredPosts(
          data.filter((post) => post.uid === uid || post.username === name)
        ); // Adjust filter based on your field
        setFilrrerUserFollowingdata(
          data1.filter(
            (userFollowingdata) => userFollowingdata.userName === name
          )
        );
        ...
        <div className="flex w-screen h-full justify-between">
          <div className="flex w-screen h-max lg:flex-row flex-col gap-[200px] justify-center items-center">
            <div className="h-max w-[30%] flex flex-col items-center justify-between gap-[400px]">
              ...
              {console.log(
                FilteredUserFollowingdata?.followers?.length +
                  FilteredUserFollowingdata +
                  FilteredUserFollowingdata[0]?.followers?.length +
                  FilteredUserFollowingdata[0]?.followers.length
              )}
            </div>
          </div>
