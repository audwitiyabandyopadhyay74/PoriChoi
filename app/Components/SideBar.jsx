...
const SideBar = () => {
  ...
  return (
    <div className="flex flex-col sm:flex-row h-full sm:h-screen w-full sm:w-1/4 bg-gray-800 sm:rounded-md">
      <ul className="w-full">
        <li className={profile ? active : notActive} onClick={() => { document.location.href = '/profile' }}>
          <i className='fa-solid fa-user' style={{ fontSize: '24px' }}></i> Your Profile
        </li>
        <li className={settings ? active : notActive} onClick={() => { document.location.href = '/profile/settings' }}>
